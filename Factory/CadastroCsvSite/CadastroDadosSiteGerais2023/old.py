...
class CadastroDadosSiteGerais2023(Processo):
    ...
    def executar(self, Model, View):
        ...
        for row in self.data_base_table_cvs_relatorio_geral_clientes_2023:
            ...
            if row != 'parada':
                ...
                total_recebido = self.somar_valores(list_value=str(row[10]).split('*'))
                saldo_atrasado = self.somar_valores(list_value=str(row[12]).split('*'))
                ...
                if saldo_a_vencer != '0':
                    pass
                parcelas_vencidas = self.extrair_bb(nome_cliente=str(nome_cliente),data_frame_devedores=self.data_base_table_csv_dados_de_pagamento_padrao)
                parcelas_a_vencer = self.extrair_total_parcelas_menos_devendo(nome_cliente=str(nome_cliente),data_frame_devedores=self.data_base_table_csv_dados_de_pagamento_padrao)
    ...
    def extrair_total_parcelas_menos_devendo(self, nome_cliente: str, data_frame_devedores):
        ...
        if lista_de_parcelas_em_aberto.__len__() != 0:
            ...
            if nome_cliente == 'ANTONIO AUGUSTO HELRIGHEL RIBEIRO':
                pass
    ...
    def extrair_bb(self, nome_cliente: str, data_frame_devedores) -> str:
        ...
        if lista_de_parcelas_em_aberto.__len__() != 0:
            lista_de_parcelas_em_aberto.reverse()
            ...
            parcela_final = int(dict_parcela_final['parcelas_devedor'].split('/')[1])
            valor_parcela_final = (parcela_final - parcela_inicial)
            if nome_cliente == 'ANTONIO AUGUSTO HELRIGHEL RIBEIRO':
                pass
